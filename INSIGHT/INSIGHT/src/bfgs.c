/** 
   \file bfgs.c 
   Implementing BFGS optimization procedure used for maximization of a function.
   ...
   The only interface function is setULbounds() [ see documentation below ]
			     ...
			     int *isave, double *dsave)
{
    ...
    /* Local variables */
    static integer lsnd, lsgo, lygo, /*--- UNUSED l1, l2, l3, ---*/ ld, lr, lt;
...
int mainlb_(n, m, x, l, u, nbd, f, g, factr, pgtol, ws, wy, 
...
/*                                terminate the algorithm. */
      myStrcpy(task, "CONVERGENCE: NORM OF PROJECTED GRADIENT <= PGTOL");
...
/*                restore the actual number of f and g evaluations etc. */
	    ...
	    myStrcpy(task, "ABNORMAL_TERMINATION_IN_LNSRCH");
...
/*             refresh the lbfgs memory and restart the iteration. */
	  if (*iprint >= 1) {
	    ...
	    iupdat = 0;
	    updatd = FALSE_;
	    myStrcpy(task, "RESTART_FROM_LNSRCH");
...
/* Computing MAX */
    ...
    if (fold - *f <= tol * ddum) {
/*                                        terminate the algorithm. */
      myStrcpy(task, "CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH");
	if (iback >= 10) {
	    info = -5;
	}
...
int active_(n, l, u, nbd, x, iwhere, iprint, prjctd, cnstnd, 
	boxed)
integer *n;
doublereal *l, *u;
integer *nbd;
...
int cauchy_(n, x, l, u, nbd, g, iorder, iwhere, t, d__, xcp, 
...
int cmprlb_(n, m, x, g, ws, wy, sy, wt, z__, r__, wa, index, 
...
int errclb_(n, m, factr, l, u, nbd, task, info, k, task_len)
integer *n, *m;
...
/*                                                   return */
	  myStrcpy(task, "ERROR: INVALID NBD");
	...
	if (nbd[i__] == 2) {
	    if (l[i__] > u[i__]) {
/*                                    return */
	      myStrcpy(task, "ERROR: NO FEASIBLE SOLUTION");
...
int formk_(n, nsub, ind, nenter, ileave, indx2, iupdat, 
...
int formt_(m, wt, sy, ss, col, theta, info)
integer *m;
doublereal *wt, *sy, *ss;
integer *col;
doublereal *theta;
...
int freev_(n, nfree, index, nenter, ileave, indx2, iwhere, 
	wrk, updatd, cnstnd, iprint, iter)
...
int lnsrlb_(n, l, u, nbd, x, f, fold, gd, gdold, g, d__, r__,
...
int matupd_(n, m, ws, wy, sy, ss, d__, r__, itail, iupdat, 
...
int prn1lb_(n, m, l, u, x, iprint, itfile, epsmch)
integer *n, *m;
doublereal *l, *u, *x;
...
int prn2lb_(n, x, f, g, iprint, itfile, iter, nfgv, nact, 
...
int prn3lb_(n, x, f, task, iprint, info, itfile, iter, nfgv, 
...
int projgr_(n, l, u, nbd, x, g, sbgnrm)
integer *n;
doublereal *l, *u;
integer *nbd;
...
int subsm_(n, m, nsub, ind, l, u, nbd, x, d__, ws, wy, theta,
...
int dcsrch_(f, g, stp, ftol, gtol, xtol, stpmin, stpmax, 
...
/*        Check the input arguments for errors. */
	if (*stp < *stpmin) {
	    myStrcpy(task, "ERROR: STP .LT. STPMIN");
	}
	if (*stp > *stpmax) {
	    myStrcpy(task, "ERROR: STP .GT. STPMAX");
	}
	if (*g >= 0.) {
	    myStrcpy(task, "ERROR: INITIAL G .GE. ZERO");
	}
	if (*ftol < 0.) {
	    myStrcpy(task, "ERROR: FTOL .LT. ZERO");
	}
	if (*gtol < 0.) {
	    myStrcpy(task, "ERROR: GTOL .LT. ZERO");
	}
	if (*xtol < 0.) {
	  myStrcpy(task, "ERROR: XTOL .LT. ZERO");
	}
	if (*stpmin < 0.) {
	  myStrcpy(task, "ERROR: STPMIN .LT. ZERO");
	}
	if (*stpmax < *stpmin) {
	  myStrcpy(task, "ERROR: STPMAX .LT. STPMIN");
	}
...
/*     Test for warnings. */
    if (brackt && (*stp <= stmin || *stp >= stmax)) {
      myStrcpy(task, "WARNING: ROUNDING ERRORS PREVENT PROGRESS");
    }
    if (brackt && stmax - stmin <= *xtol * stmax) {
	myStrcpy(task, "WARNING: XTOL TEST SATISFIED");
    }
    if (*stp == *stpmax && *f <= ftest && *g <= gtest) {
	myStrcpy(task, "WARNING: STP = STPMAX");
    }
    if (*stp == *stpmin && (*f > ftest || *g >= gtest)) {
	myStrcpy(task, "WARNING: STP = STPMIN");
    }
/*     Test for convergence. */
    if (*f <= ftest && abs(*g) <= *gtol * (-ginit)) {
	myStrcpy(task, "CONVERGENCE");
    }
/*     Test for termination. */
...
int dcstep_(stx, fx, dx, sty, fy, dy, stp, fp, dp, brackt, 
	stpmin, stpmax)
